...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 20 08:42:20 2025
...
def unpas_de_dt():
    for j in range(np):
        ...
        # Rebond sur le sol thermostaté
        if x[j] < 0:
            x[j] = -x[j]
            v[j] = abs(rd.gauss(0, sqrt(T0)))  # vitesse vers le haut
...
def echange_vitesse():
    ...
    for j in range(np):
        dvj = rd.uniform(-dv, dv)
        v[j] += dvj
    E_apres = sum(0.5 * m * vj ** 2 for vj in v)
    alpha = sqrt(E_avant / E_apres) if E_apres > 0 else 1.0
    for j in range(np):
        v[j] *= alpha
