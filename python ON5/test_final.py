...
class Objets_Celestes:
    ...
    def calculs_positions(self, i, dt):
        ...
        acc_x, acc_y, contact2 = self.acceleration(objets_celestes, self.x[i-1] + 0.5 * dt * k1_x, self.y[i-1] + 0.5 * dt * k1_y, i-1)
        k2_x_point = acc_x
        k2_y_point = acc_y
        ...
        acc_x, acc_y, contact3 = self.acceleration(objets_celestes, self.x[i - 1] + 0.5 * dt * k2_x, self.y[i - 1] + 0.5 * dt * k2_y, i-1)
        k3_x_point = acc_x
        k3_y_point = acc_y
        ...
        acc_x, acc_y, contact4 = self.acceleration(objets_celestes, self.x[i - 1] + dt * k3_x, self.y[i - 1] + dt * k3_y, i-1)
        k4_x_point = acc_x
        k4_y_point = acc_y
        ...
        if ((contact1 == True) or (contact2 == True) or (contact3 == True) or (contact4 == True)) and i>1:
            ...
            if cont[0] != nouveau_i[0]-1:
                ...
                print('ici', i, t)
            self.x[i] = (contact_avec[0].rayon+self.rayon)*np.cos(contact_avec[0].vit_siderale*(tmax/Nt)*(nouveau_i[0]))
            self.y[i] = (contact_avec[0].rayon+self.rayon)*np.sin(contact_avec[0].vit_siderale*(tmax/Nt)*(nouveau_i[0]))

            self.x_point[i] = -(contact_avec[0].rayon+self.rayon)*contact_avec[0].vit_siderale * np.sin(contact_avec[0].vit_siderale * (tmax / Nt) * (nouveau_i[0]))
            self.y_point[i] = (contact_avec[0].rayon+self.rayon)*contact_avec[0].vit_siderale * np.cos(contact_avec[0].vit_siderale * (tmax / Nt) * (nouveau_i[0]))
        else:
            self.x[i] = self.x[i - 1] + (dt / 6) * (k1_x + 2 * k2_x + 2 * k3_x + k4_x)
            self.y[i] = self.y[i - 1] + (dt / 6) * (k1_y + 2 * k2_y + 2 * k3_y + k4_y)

            self.x_point[i] = self.x_point[i - 1] + (dt / 6) * (k1_x_point + 2 * k2_x_point + 2 * k3_x_point + k4_x_point)
            self.y_point[i] = self.y_point[i - 1] + (dt / 6) * (k1_y_point + 2 * k2_y_point + 2 * k3_y_point + k4_y_point)

        if i != i_davant[0]:
            i_davant[0] = i
            nouveau_i[0] += 1

        if i>=1620 and i<8686 and self.nom=='Fusée':
            test_liste.append(fusee.x[i])
            test_liste2.append(fusee.y[i])
            test_liste.append(fusee.x[i] + (1 / 6) * (k1_x_point + 2 * k2_x_point + 2 * k3_x_point + k4_x_point))
            test_liste2.append(fusee.y[i] + (1 / 6) * (k1_y_point + 2 * k2_y_point + 2 * k3_y_point + k4_y_point))
    ...
    def acceleration(self, objets_celestes, x, y, n):
        ...
        if nb_etages_pop[0] < 2:
            ...
            if nb_etages[nb_etages_pop[0]].masse[n] <= nb_etages[nb_etages_pop[0]].masse_vide:
                print('#########################################')
                print('On est a', ((fusee.x[n]-terre.x[n])**2+(fusee.y[n]-terre.y[n])**2)**0.5 -terre.rayon, 'm')
                print('#########################################')
                print('vitesse nécessaire=', (np.sqrt((G*terre.masse)/(((fusee.x[n]-terre.x[n])**2+(fusee.y[n]-terre.y[n])**2)**0.5))))
            ...
            nb_etages[nb_etages_pop[0]].masse[n] = nb_etages[nb_etages_pop[0]].masse[n - 1] - conso_par_pas
            if nb_etages[nb_etages_pop[0]].masse[n] <= (136*(nb_etages[nb_etages_pop[0]].masse[0]-nb_etages[nb_etages_pop[0]].masse_vide)/nb_etages[nb_etages_pop[0]].duree_fct)+nb_etages[nb_etages_pop[0]].masse_vide:
                print('#########################################')
                print('On est a', ((fusee.x[n]-terre.x[n])**2+(fusee.y[n]-terre.y[n])**2)**0.5 -terre.rayon, 'm')
                print('#########################################')
                print('vitesse nécessaire=', (np.sqrt((G*terre.masse)/(((fusee.x[n]-terre.x[n])**2+(fusee.y[n]-terre.y[n])**2)**0.5))))
                print('vitesse actuelle=', (fusee.x_point[n] ** 2 + fusee.y_point[n] ** 2) ** 0.5, fusee.x_point[n], fusee.y_point[n])
                liste_plots_fusee.append(n)
                u[0]=1
        ...
        if (acc_x**2+acc_y**2)**0.5 != 0:
            cos_alpha = -acc_x/norme_acc - np.pi/15
        ...
        if etage_actuel == 1 and n>1:
            if int(sec_par_pas * n) <= 20:
                acc_x += Fp
            elif int(sec_par_pas * n) <= 150:
                acc_x += Fp * np.cos(np.pi/7)
                acc_y += Fp * np.sin(np.pi/7)
            elif int(sec_par_pas * n) > 150:
                acc_x += Fp * np.cos(np.pi/3)
                acc_y += Fp * np.sin(np.pi/3)

        if etage_actuel == 2:  # Arrivée ici, on est a 160 secondes
    
            if int(sec_par_pas * n) < 305:
        ...
        if etage_actuel == 3 and ((fusee.x_point[n]**2+fusee.y_point[n]**2)**0.5)<=(np.sqrt((G*terre.masse)/(((fusee.x[n]-terre.x[n])**2+(fusee.y[n]-terre.y[n])**2)**0.5))) and u[0]!=1:
            if int(sec_par_pas * n) < 550: print(int(sec_par_pas * n))
            test_lis.append(fusee.x[n])
            test_lis2.append(fusee.y[n])
            test_lis.append(fusee.x[n] + acc_x)
            test_lis2.append(fusee.y[n] + acc_y)
            if int(sec_par_pas*n) > 500:
                acc_x += (-norme_tan*sin_alpha + norme_acc*cos_alpha)  # Adapte le force de poussée pour que la fusée aille sur la tangente
                acc_y += (norme_tan*cos_alpha + norme_acc*sin_alpha)
            else:
                acc_x += Fp * np.cos(3*np.pi / 4)
                acc_y += Fp * np.sin(3*np.pi / 4)
            test_list.append(fusee.x[n])
            test_list2.append(fusee.y[n])
            test_list.append(fusee.x[n] + (-norme_tan*sin_alpha + norme_acc*cos_alpha))
            test_list2.append(fusee.y[n] + (norme_tan*cos_alpha + norme_acc*sin_alpha))
...
def plot_fusee(t, nb_etages_pop):
    ...
    if t==0: x_arr, y_arr = terre.x[t] + (r-L)*cos0, terre.y[t] + (r-L)*sin0
    else: x_arr, y_arr = fusee.x[t-1] + (r-L)*cos0, fusee.y[t-1] + (r-L)*sin0
    ...
    for i, elem in enumerate(nb_etages[nb_etages_pop:]):
        sommet = [(x_arr + elem.largeur*sin0/2 + longueur[i]*cos0, y_arr - elem.largeur*cos0/2 + longueur[i]*sin0),
                  (x_arr - elem.largeur*sin0/2 + longueur[i]*cos0, y_arr + elem.largeur*cos0/2 + longueur[i]*sin0),
                  (x_arr - elem.largeur*sin0/2 + longueur[i+1]*cos0, y_arr + elem.largeur*cos0/2 + longueur[i+1]*sin0),
                  (x_arr + elem.largeur*sin0/2 + longueur[i+1]*cos0, y_arr - elem.largeur*cos0/2 + longueur[i+1]*sin0)]
...
def plot_final(color, x, y, x_point, y_point, nom, rayon, pos_text):
    ...
    if nom == 'Terre':
        for i in range(len(liste_plots_fusee)): axs[0].add_artist(plt.Circle((terre.x[liste_plots_fusee[i]], terre.y[liste_plots_fusee[i]]), rayon, color='b', alpha=0.7))
...
terre = Objets_Celestes('Terre', 5.972*10**24, 6371*10**3, 7.2921159*10**(-5), 'b', 0.0, 0.0, 0.0, vit_terre, Nt)
...
vit_fusee_theorique = (np.sqrt((G * terre.masse) / (terre.rayon+180000)))
fusee_theorique = Objets_Celestes('fusee_theorique', masse_fusee, fusee.rayon, 16.7/3.6, 'b', terre.rayon+180000, 0.0, 0.0, vit_fusee_theorique,  Nt)
...
for i in range(1, Nt):
    ...
    if i==1:
        ...
        execution_time_estimation = (end_time_estimation - start_time_estimation)*(Nt-i)  # Calculer la durée totale d'exécution
        if execution_time_estimation >= 60:
            print(f"Il reste environ : {int(execution_time_estimation/60)}{min} et {(execution_time_estimation/60%1)*60:.6f}", sec)
        else: print(f"Il reste environ : {execution_time_estimation:.6f}", sec)
...
axs[0].set_xlim(-dist_terre_lune, dist_terre_lune)
axs[0].set_ylim(-dist_terre_lune, dist_terre_lune)
...
axs[0].plot(test_liste, test_liste2, 'go-')
axs[0].plot(test_list, test_list2, 'mo-')
axs[0].plot(test_lis, test_lis2, 'bo-')
axs[0].plot(fusee.x[3151], fusee.y[3151], 'yo')
plt.tight_layout()
plt.show()
"""
