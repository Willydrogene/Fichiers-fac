...
class Objets_Celestes:
    ...
    def calculs_positions(self, i, dt):
        ...
        if i>=2023 and i<6000 and self.nom=='Fusée':
            test_liste.append(fusee.x[i])
            test_liste2.append(fusee.y[i])
            test_liste.append(fusee.x[i] + (k1_x_point))
            test_liste2.append(fusee.y[i] + (k1_y_point))
            print('JE SUIS ICI')
...
class Fusee(Objets_Celestes):
    def acceleration(self, objets_celestes, x, y, n):
        ...
        if (acc_x**2+acc_y**2)**0.5 != 0:
            OA = ((acc_x**2+(acc_y+(terre.rayon*terre.vit_siderale**2))**2) ** 0.5)/16  # Norme du vecteur acc de pesanteur
            OC = Fp  # Norme du vecteur Fp
            theta = np.arccos(-acc_x/(acc_x**2+acc_y**2)**0.5)  # Angle du vecteur accélération vers la Terre
            #print(n, OA, OC, theta, r_fusee_2, (fusee.x_point[n-1]**2+fusee.y_point[n-1]**2)**0.5)
        else: OA, OC = 3, 1
        ...
        if etage_actuel == 2 and OA<OC:
            if n  >= 2023 and n<6000:
                print(fusee.x[n])
                print(acc_x)
            ...
            acc_x += Fp * np.sin(np.arcsin(OA/OC) - theta)  # Adapte le force de poussée pour que la fusée aille sur la tangente
            acc_y += Fp * np.cos(np.arcsin(OA/OC) - theta)
            
            if n  >= 2023 and n<6000:
                test_list.append(fusee.x[n])
                test_list2.append(fusee.y[n])
                test_list.append(fusee.x[n] + Fp * np.sin(np.arcsin(OA/OC) - theta))
                test_list2.append(fusee.y[n] + Fp * np.cos(np.arcsin(OA/OC) - theta))
        ...
        #if n<100: print('ON A CA:::::::::          ', acc_x, acc_y)
        for i, objet in enumerate(objets_celestes):
