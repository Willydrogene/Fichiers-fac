    ...
    def acceleration(self, objets_celestes, x, y, n):
        ...
        if nb_etages_pop[0] < 2:
            ...
            if nb_etages[nb_etages_pop[0]].masse[n] <= nb_etages[nb_etages_pop[0]].masse_vide:
                print('#########################################')
                print('On est a', ((fusee.x[n]-terre.x[n])**2+(fusee.y[n]-terre.y[n])**2)**0.5 -terre.rayon, "m, l'étage", nb_etages_pop[0]+1, "est largué")
                print('#########################################')
        ...
        if etage_actuel == 1 and n>1:
            angle1 = int(sec_par_pas * n) * (np.pi/8)/80
            angle2 = int(sec_par_pas * n) * (np.pi/2)/138
            if int(sec_par_pas * n) <= 12:
                acc_x += Fp * np.cos(np.pi/20)
                acc_y += Fp * np.sin(np.pi/20)
            
            elif int(sec_par_pas * n) <= 80:
                acc_x += Fp * np.cos(angle1)
                acc_y += Fp * np.sin(angle1)
                
            elif int(sec_par_pas * n) <= 160:
                acc_x += Fp * np.cos(angle2)
                acc_y += Fp * np.sin(angle2)
                

        if etage_actuel == 2:
            if int(sec_par_pas * n) <= 250:
...
tmin = 0
tmax = 1000
dt = 0.01  # mettre 1000 pour 365jours
...
axs[0].plot(fusee.x[7787], fusee.y[7787], 'yo')
axs[1].plot(fusee.x_point[16154], fusee.y_point[16154], 'yo')
plt.tight_layout()
plt.show()
"""
