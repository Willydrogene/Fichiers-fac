...
tmin = 0
tmax = 3600 * 24 * 365
dt = 1000  # mettre 1000 pour 365jours
...
soleil = Objets_Celestes('Soleil', 2 * 10 ** 30, 696340 * 10 ** 3, 30 * 10 ** 3, '#FFFF00', 0.0, 0.0, 0.0, 0.0, Nt)
...
lune = Objets_Celestes('Lune', 7.36 * 10 ** 22, 1737.4 * 10 ** 3, 16.7 / 3.6, '#D3D3D3', dist_terre_soleil + dist_terre_lune,
                       0.0, 0.0, vit_lune, Nt)

vit_mercure = (np.sqrt((G * soleil.masse) / (63.524 * 10 ** 9)))
mercure = Objets_Celestes('Mercure', 3.285*10**23, 2439.7*10**3, 10.892/3.6, '#A9A9A9', 63.524*10**9, 0.0, 0.0, vit_mercure, Nt)

vit_venus = (np.sqrt((G * soleil.masse) / (108 * 10 ** 9)))
venus = Objets_Celestes('Vénus', 4.869*10**24, 6051.8*10**3, 10.892/3.6, '#FFD700', 108*10**9, 0.0, 0.0, vit_venus, Nt)

vit_mars = (np.sqrt((G * soleil.masse) / (228 * 10 ** 9)))
mars = Objets_Celestes('Mars', 6.417*10**23, 3389.5*10**3, 10.892/3.6, '#FF4500', 228*10**9, 0.0, 0.0, vit_mars, Nt)

vit_jupiter = (np.sqrt((G * soleil.masse) / (780 * 10 ** 9)))
jupiter = Objets_Celestes('Jupiter', 1.89813*10**27, 69911*10**3, 10.892/3.6, '#8B4513', 780*10**9, 0.0, 0.0, vit_jupiter, Nt)

vit_saturne = (np.sqrt((G * soleil.masse) / (1430 * 10 ** 9)))
saturne = Objets_Celestes('Saturne', 5.683*10**26, 58232*10**3, 10.892/3.6, '#D2B48C', 1430*10**9, 0.0, 0.0, vit_saturne, Nt)

vit_uranus = (np.sqrt((G * soleil.masse) / (2800 * 10 ** 9)))
uranus = Objets_Celestes('Uranus', 8.681*10**25, 25362*10**3, 10.892/3.6, '#A0E6FF', 2800*10**9, 0.0, 0.0, vit_uranus, Nt)

vit_neptune = (np.sqrt((G * soleil.masse) / (4504.3 * 10 ** 9)))
neptune = Objets_Celestes('Neptune', 1.024*10**26, 24622*10**3, 10.892/3.6, '#21498E', 4505.3*10**9, 0.0, 0.0, vit_neptune, Nt)
...
axs[0].set_xlim(-(mars.rayon+mars.x[0]), neptune.rayon*2+neptune.x[0])
axs[0].set_ylim(-(mars.rayon+mars.x[0]), neptune.rayon*2+neptune.x[0])
