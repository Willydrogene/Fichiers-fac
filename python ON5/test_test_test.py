...
# -*- coding: utf-8 -*-
"""
Created on Tue May 14 17:03:16 2024
...
class Fusee(Objets_Celestes):
    def acceleration(self, objets_celestes, x, y, n):
        ...
        if r_fusee_1 < r_fusee_2 and etage_actuel >1 and r_fusee_1>(140000+terre.rayon):
            theta[0] = np.pi
            
            if len(test_liste)==0: 
                test_liste.append(n)
                print('ohhh', n, r_fusee_1 - r_fusee_2)
        elif r_fusee_1 > r_fusee_2 and etage_actuel >1 and r_fusee_1>(140000+terre.rayon):
            theta[0] = np.pi/4
            
            if len(test_liste2)==0: 
                test_liste2.append(n)
                print('nan', n, r_fusee_1 - r_fusee_2)
...
axs[0].plot(fusee.x[test_liste[0]], fusee.y[test_liste[0]], 'bo')
axs[0].plot(fusee.x[test_liste2[0]], fusee.y[test_liste2[0]], 'go')
axs[0].plot(fusee.x[test_liste[0]+1], fusee.y[test_liste[0]+1], 'bo')
axs[0].plot(fusee.x[test_liste2[0]+1], fusee.y[test_liste2[0]+1], 'go')

axs[1].plot(fusee.x_point[test_liste[0]], fusee.y_point[test_liste[0]], 'bo')
axs[1].plot(fusee.x_point[test_liste2[0]], fusee.y_point[test_liste2[0]], 'go')
